...
Tecnicamente, o Vue.js é focado na camada [ViewModel](#ViewModel) do padrão MVVM. O ViewModel é responsável por conectar a [View](#View) e o [Model](#Model) através de um mecanismo chamado *two way data binding*. Manipulações no DOM  e a formatação das saídas são abstraídas utilizando [Diretivas](#Diretivas) e [Filtros](#Filtros).

Filosoficamente, o objetivo é fornecer benefícios do *reactive data binding* e componentes que possam ser combinados, baseados em uma API que seja o mais simples possível. Não é um framework completo - é desenvolvido para ser a camada *view*, sendo simples e flexível. Você pode utilizar ele somente para uma rápida prototipagem, ou combinar com outras bibliotecas para obter uma *stack de front end* customizada. Também se encaixa para serviços *no-backend* como o Firebase.
...
Um objeto que sincroniza o *Model* e a *View*. No Vue.js, cada instância do Vue é um ViewModel. Eles são instanciados com construtor do `Vue` ou suas sub-classes:
...
No Vue.js, *models* são apenas objetos Javascript simples, ou **data objects**. A partir do momento que o objeto é utilizado como dados em uma instância do Vue, ele se torna **reativo**. Você pode manipular as propriedades deles e as instâncias do Vue que estarão observando esses objetos serão notificados das mudanças. O Vue.js alcança uma reatividade transparente convertendo as propriedades em *data objects* utilizando getter/setters do ES5. Não há necessidade para checagens mais "poluídas", e você também não precisa chamar explicitamente o Vue para atualizar a View. Não importa quando os dados mudam, a View sempre será atualizada no próximo frame.

As instâncias do Vue interceptarão e observarão todas as propriedades nos *data objects*. Então a partir do momento em que um objeto `{ a: 1 }` está sendo observado, tanto `vm.$data.a` quanto `vm.a` retornarão o mesmo valor, e ao setar um valor como `vm.a = 2` também modificará o valor de `vm.$data`.

Os *data objects* são modificados em um único local, então modificá-lo via referência tem o mesmo efeito que modificar `vm.$data`. Isso faz possível com que múltiplas instâncias do Vue observem um mesma porção de dados. Em grandes aplicações também é recomendado tratar instâncias do Vue como simples *views*, assim externalizando a lógica na manipulação dos dados para um layer de armazenamento mais discreto.
...
A imagem abaixo demonstra uma visão geral mais detalhada de como os updates *reativos* são implementados no Vue.js:
...
Diretivas podem encapsular manipulações arbitrárias no DOM. Por exemplo, a diretiva `v-attr` manipula os atributos de um elemento, a diretiva `v-repeat` clona um elemento baseado em um Array, e a diretiva `v-on` inclui *event listeners*... nós falaremos disso depois.

### Mustache Bindings

 Você também pode usar *mustache-style bindings*. Eles serão traduzidos para as diretivas `v-text` e `v-attr` directives under the hood. For example:
...
Finalmente, você pode adicionar um `*` ao seu *mustache binding* para indicar que essa é uma interpolação única, e que não será reativa para mudanças:
...
Você pode clicar em um todo para alternar seu estado, ou você pode abrir o console de seu navegador e brincar com o objeto `demo` - por exemplo, mude `demo.title`, e adicione um novo objeto à `demo.todos`, ou alterne um status de *done* de um *todo*.
